...
Published NexPress family releases automatically dispatch the **Update
NexPress** GitHub Actions workflow with the exact version and source commit;
staff may also run it manually as a recovery path. It verifies npm provenance,
synchronizes every installed `@nexpress/*` package, generates migrations, runs
the local gates, and opens a draft PR for migration and preview review. A
successful Vercel `Production` deployment then triggers the live readiness and
homepage smoke workflow. See [Demo
Operations](./docs/demo-operations.md#nexpress-version-update-checklist).
